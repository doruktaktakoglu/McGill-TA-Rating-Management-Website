...
$sql2 = $conn->prepare(
    'SELECT FORMAT(AVG(TA_Ratings.rating),2) as ta_rating_average FROM TA_Ratings WHERE TA_Ratings.ta_email = (SELECT DISTINCT email FROM TA WHERE ta.email = ?)'
);
$sql2->bind_param('s', $ta_email);
$sql2->execute();

$ta_averges = $sql2->get_result();

$sql3 = $conn->prepare(
    'SELECT COUNT(course) as total_courses FROM `TA` WHERE email = ?;'
);

$sql3->bind_param('s', $ta_email);
$sql3->execute();
