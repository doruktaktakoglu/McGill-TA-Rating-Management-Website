...
function fillEmail(role) {
  ...
  const ta_out =   '<option value="' + ta + '">' + ta + '</option>';
  const prof_out =   '<option value="' + prof + '">' + prof + '</option>';
  if (role == "ta") {
    // console.log(ta);
    populateEmailTableTa(ta_out, "email");
  } else {
    // console.log(prof);
    populateEmailTableProf(prof_out, "email");
  }
}
